...
class SFC extends EventEmitter {
  ...
  async normalizeFile(input, outFile) {
    ...
    if (!input.endsWith('.vue')) {
      return this.writeNormal(source, ctx)
    }
  ...
  async writeNormal(source, { filename, babelrc, modern, outFile }) {
    let output

    if (filename.endsWith('.js')) {
      output = await require('./script-compilers/babel')(source.toString(), {
        filename,
        babelrc,
        modern
      })
    } else if (filename.endsWith('.ts')) {
      output = await require('./script-compilers/ts')(source.toString(), {
        filename,
        babelrc,
        modern
      })
