...
class SFC {
  ...
  async normalize() {
    const isFile = this.options.input.endsWith('.vue')
    if (isFile) {
  ...
  async normalizeFile(input, outFile) {
    ...
    const template = await require('./compileTemplate')(sfcDescriptor.template, ctx)
    const styles = await require('./compileStyles')(sfcDescriptor.styles, ctx)
    return require('./writeSFC')({
      script,
      styles,
      template
    }, outFile)
  ...
  async normalizeDir(input, outDir) {
    ...
    await Promise.all(files.map(file => {
      return this.normalizeFile(path.join(input, file), path.join(outDir, file))
    }))
  }
}
